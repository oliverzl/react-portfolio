	...
	return (
		<>
			<div className="main-bg" ref={props.refProp}>
				{/* <button
					onClick={() =>
						setSkillsAnimationPlaying((prev) => {
							return !prev;
						})
					}
				>
				...
				<button onClick={() => setSkillsAnimationPlaying(false)}>
					set to false
				</button>
				<button style={{ fontSize: "30px" }}>
					SkillsState = {`${skillsAnimationPlaying}`}
				</button> */}
				{skillsAnimationPlaying ? (
					<div>
						...
						<Typist cursor={{ show: false }} avgTypingDelay={45}>
							...
							<h4 className="discuss">Learning more JavaScript everyday.</h4>
						</Typist>
					</div>
				) : (
					""
				...
				{/* <div className="skills-logos">
					...
					<button onClick={help1}>Restart</button>
					<button onClick={() => animationRef.current.play()}>play</button>

					<button onClick={() => animationRef.current.pause()}>pause</button>
					<img src={express} alt="" className="express logotest" />
					<DiNodejsSmall className="skills-logo nodelogo" />
					<button style={{ fontSize: "30px", marginTop: "500px" }}>
						Skills State = {`${skillsAnimationPlaying}`}
					</button>
				</div> */}
			</div>
		</>
